...
class Program
{
    static void Main()
    {
        ...
        try
        {
            ...
            var identityTables = tables.Where(t => t.StartsWith("AspNet") || t.Contains("Identity")).OrderBy(t => t).ToList();
            var garaTables = tables.Where(t => t.StartsWith("Claims") || t.StartsWith("SoftDelete")).OrderBy(t => t).ToList();
            var configTables = tables.Where(t => t.StartsWith("Clients") || t.StartsWith("Api") || t.StartsWith("IdentityResources") || t.StartsWith("ClientScopes") || t.StartsWith("ClientSecrets") || t.StartsWith("ApiScopes") || t.StartsWith("ApiScopeClaims") || t.StartsWith("ApiResourceClaims") || t.StartsWith("ApiResourceScopes") || t.StartsWith("ApiResourceSecrets")).OrderBy(t => t).ToList();
