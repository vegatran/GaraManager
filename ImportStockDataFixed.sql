-- =====================================================
-- SCRIPT IMPORT DỮ LIỆU TỒN KHO TỪ BÁO CÁO (ĐÃ SỬA THEO ENTITY)
-- =====================================================
...
('ACQUY_70', 'BÌNH ẮC QUY GS N70', 'Ắc quy GS N70 cho xe hạng trung', 
 ...
 'CÁI', 'Ắc quy', 'GS', 
 1, NOW(), NOW(), 0,
 'Purchased', 'WithInvoice', 1, 1, 1, 1, 'New', 12, 0),
...
('BAQGN', 'Bạc đạn ZA.60TB039B09', 'Bạc đạn chất lượng cao', 
 ...
 'CÁI', 'Bạc đạn', 'ZA', 
 1, NOW(), NOW(), 0,
 'Purchased', 'WithInvoice', 1, 1, 1, 1, 'New', 24, 0),
...
('DBT10W40', 'Dầu bôi trơn ENEOS AL SNCF 10W40 4L *6', 'Dầu động cơ ENEOS 10W40', 
 272727, 272727, 300000, 24, -- Sửa từ 24625 thành 24 (int)
 'Chai', 'Dầu nhớt', 'ENEOS', 
 1, NOW(), NOW(), 0,
 'Purchased', 'WithInvoice', 1, 1, 1, 1, 'New', 12, 0),
...
('SONXE2K', 'Sơn xe 2K', 'Sơn xe 2K chất lượng cao', 
 ...
 'Bộ', 'Sơn', 'Generic', 
 1, NOW(), NOW(), 0,
 'Purchased', 'WithInvoice', 1, 1, 1, 1, 'New', 6, 0),
...
('ST013', (SELECT Id FROM Parts WHERE PartNumber = 'ACQUY_70'), 1, -- NhapKho = 1
 22, 10, 32, 32,
...
('ST014', (SELECT Id FROM Parts WHERE PartNumber = 'DBT10W40'), 1, -- NhapKho = 1
 30, 0, 30, 30,
...
('ST015', (SELECT Id FROM Parts WHERE PartNumber = 'ACQUY_70'), 2, -- XuatKho = 2
 9, 32, 23, 23,
 1194340, 1300000, 10749060, 11700000, '2025-01-25 16:00:00',
 'Xuất kho - Ắc quy GS N70', NOW(), NOW(), 0);

-- Xuất trong kỳ cho Dầu động cơ: 6 chai (sửa từ 5.275 thành 6 vì Quantity là int)
INSERT INTO StockTransactions (
    TransactionNumber, PartId, TransactionType, 
    Quantity, QuantityBefore, QuantityAfter, StockAfter,
    UnitCost, UnitPrice, TotalCost, TotalAmount, TransactionDate,
...
('ST016', (SELECT Id FROM Parts WHERE PartNumber = 'DBT10W40'), 2, -- XuatKho = 2
 6, 30, 24, 24,
...
UPDATE Parts SET QuantityInStock = 24 WHERE PartNumber = 'DBT10W40'; -- Sửa từ 24.625 thành 24
...
-- Kiểm tra Stock Transactions
SELECT 
    st.TransactionNumber,
    p.PartNumber,
    CASE st.TransactionType 
        WHEN 1 THEN 'Nhập kho'
        WHEN 2 THEN 'Xuất kho' 
        WHEN 3 THEN 'Điều chỉnh'
        WHEN 4 THEN 'Tồn đầu kỳ'
        ELSE 'Unknown'
    END as 'Loại Giao Dịch',
    st.Quantity,
    st.QuantityBefore,
    st.QuantityAfter,
    st.UnitCost,
...
JOIN Parts p ON st.PartId = p.Id
WHERE p.PartNumber IN (
    'ACQUY_70', 'BAQGN', 'BDZA', 'DBT10W40', 'MABOSCH', 
    'LOTP205', 'SONXE2K', 'DAUPHANH', 'BUGINGK', 
    'DAYCUROA', 'LOCGIO', 'DAUHOPSO'
)
ORDER BY st.TransactionDate, st.TransactionNumber;
